...
def main():
    ...
    parser.add_argument('--data_name', type=str,
                        help=('which version of data will be used? (20210712 Or 20210710?)'))

    parser.add_argument('--metadata_dir', type=str,
                        help=('directory to load the metadata.'))
    ...
    if args.data_name is not None and args.data_dir is not None:
        ...
        raise argparse.ArgumentTypeError("data_dir can't be recognized.")
    ...
    if args.metadata_dir is not None and args.metadata_file is not None:
        metadata_path = os.path.join(args.metadata_dir, args.metadata_file)
    else:
        raise argparse.ArgumentTypeError(args.metadata_dir + ' or ' + args.metadata_file + " can't be recognized.")
    ...
    output_list = [y_train, X_train_struc, X_train_text, y_dev, X_dev_struc, X_dev_text]
    path_name_list = ['y_train', 'X_train_struc', 'X_train_text', 'y_dev', 'X_dev_struc', 'X_dev_text']

    ### encode test data if args.test_file exist ###
    if args.test_file:
        print('*' * 50)
        ...
        output_list.extend([y_test, X_test_struc, X_test_text])
        path_name_list.extend(['y_test', 'X_test_struc', 'X_test_text'])
    ...
    print('Saved all encoded inputs!')
...
def encode_y(metadata, df_y, y_encoder=None):
    ...
    if metadata['output_type'] == 'multi_task':
        ...
        y_arr = df_y.values
        # print('*' * 20)
        # print('y_arr is in shape {}'.format(y_arr.shape))
        # print('*' * 20)

        if y_arr.shape[0] > 1: # if the task is multi-output binary classification
            y = y_arr
            y_encoder = None
        else:
            if y_encoder is None: # in training stage
